...
public class ProductoController {
    ...
    @FXML
    private Pagination pagination; // AGREGADO: Control de paginación
    @FXML
    ...
    ObservableList<Producto> productosFiltrados; // AGREGADO: Lista filtrada para paginación
    Producto formulario;
    Long idProductoCE=0L;
    private Timeline searchDelay; // AGREGADO: Timeline para debounce
    private static final int PAGE_SIZE = 25; // AGREGADO: Tamaño de página

    // AGREGADO: Método para crear páginas
    private Node createPage(int pageIndex) {
        ObservableList<Producto> currentList = (productosFiltrados != null) ? productosFiltrados : listarProducto;
        int from = pageIndex * PAGE_SIZE;
        int to = Math.min(from + PAGE_SIZE, currentList.size());

        if (from < currentList.size()) {
            List<Producto> pageItems = currentList.subList(from, to);
            tableView.getItems().clear();
            tableView.getItems().addAll(pageItems);
        } else {
            tableView.getItems().clear();
        }
        return tableView;
    ...
    // AGREGADO: Método para inicializar paginación
    private void initPagination() {
        ObservableList<Producto> currentList = (productosFiltrados != null) ? productosFiltrados : listarProducto;
        int pageCount = (int) Math.ceil((double) currentList.size() / PAGE_SIZE);
        pagination.setPageCount(Math.max(pageCount, 1));
        pagination.setPageFactory(this::createPage);
        pagination.setCurrentPageIndex(0); // Volver a la primera página
    }

    private void filtrarProductos(String filtro) {
        if (filtro == null || filtro.isEmpty()) {
            productosFiltrados = null; // MODIFICADO: Usar null para indicar sin filtro
        } else {
            ...
            productosFiltrados = FXCollections.observableArrayList(filtrados); // MODIFICADO: Guardar lista filtrada
        }
        initPagination(); // AGREGADO: Actualizar paginación después del filtro
    }

    public void listar(){
        try {
            listarProducto = FXCollections.observableArrayList(ps.findAll());
            productosFiltrados = null; // AGREGADO: Reset de filtros
            initPagination(); // AGREGADO: Inicializar paginación

            // MODIFICADO: Agregado debounce de 300ms con actualización de paginación
    ...
    // ... resto de métodos sin cambios (limpiarError, clearForm, editForm, etc.)
